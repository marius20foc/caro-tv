...
export default function AdminVideosPanel({ token }: { token: string }) {
  ...
  const translate = async (video: AdminVideo) => {
    setTranslatingId(video.id);
    setError('');
    try {
      const res = await fetch('/api/admin/videos/translate', {
        method: 'POST',
        headers: { 'content-type': 'application/json', authorization: `Bearer ${token}` },
        body: JSON.stringify({ id: video.id }),
      });
      ...
      if (data.skipped) {
        setNotice(`Skipped: ${data.reason ?? 'nu necesita traducere'}`);
      } else {
        setNotice('Descriere tradusă ✓');
      }
      window.setTimeout(() => setNotice(''), 3000);
      load();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Eroare la traducere');
    } finally {
                    ...
                    <div className="flex items-center justify-end gap-2">
                      {!v.description_ro ? (
                        <button
                          type="button"
                          onClick={() => translate(v)}
                          disabled={translatingId === v.id}
                          className="font-orbitron text-[10px] font-bold uppercase tracking-wider text-neon-cyan hover:underline disabled:opacity-50"
                        >
                          {translatingId === v.id ? 'Se traduce…' : 'Tradu'}
                        </button>
                      ) : (
                        <span className="font-orbitron text-[10px] uppercase tracking-wider text-ink-faint" title="Descriere tradusă">
                          RO ✓
                        </span>
                      )}
                      <button
