...
/** GET /api/admin/overrides (Pages Functions pure) */
...
export async function onRequestPut(context) {
  ...
  try {
    ...
    const video = await context.env.DB.prepare(`SELECT id FROM videos WHERE youtube_id = ?`)
      .bind(youtubeId)
      .first();
    if (!video) {
...
export async function onRequestDelete(context) {
  ...
  try {
    ...
    await context.env.DB.prepare(`DELETE FROM video_product_overrides WHERE video_id = ?`)
      .bind(videoId)
      .run();
